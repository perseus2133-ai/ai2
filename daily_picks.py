...
# -*- coding: utf-8 -*-
"""
AI 데일리 3선 — 매일 크롤 후 종합 점수로 3종목을 선정해 누적 기록.
...
def generate_daily_picks(df, snapshot_dir, history_path, today=None, top_n=TOP_N):
    ...
    short = pool.nlargest(SHORTLIST_N, '_prelim').copy()

    # ── 숏리스트만 EV보정 Upside 계산 ──
    try:
        from industry_multiple import compute_for_target
        ups = []
        for _, r in short.iterrows():
            try:
                res = compute_for_target(r, df)
